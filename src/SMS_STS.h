...
public:
	...
	virtual int WritePosEx(u8 ID, s16 Position, u16 Speed, u8 ACC = 0);//位置制御コマンド
	virtual int RegWritePosEx(u8 ID, s16 Position, u16 Speed, u8 ACC = 0);//非同期位置制御コマンド
	virtual void SyncWritePosEx(u8 ID[], u8 IDN, s16 Position[], u16 Speed[], u8 ACC[]);//複数サーボ位置制御
	virtual int WheelMode(u8 ID);//一定速度モード
	virtual int WriteSpe(u8 ID, s16 Speed, u8 ACC = 0);//定速モード制御コマンド
	virtual int WriteTorq(u8 ID, s16 Torq);
	virtual int EnableTorque(u8 ID, u8 Enable);//トルク制御指
	virtual int unLockEprom(u8 ID);//eprom解锁
	virtual int LockEprom(u8 ID);//eprom加锁
	virtual int CalibrationOfs(u8 ID);//中位校准
	virtual int FeedBack(int ID);//フィードバックサーボ情報
